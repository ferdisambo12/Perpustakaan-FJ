...
public function filter4($table, $awal, $akhir) {
	...
	public function filter_history($table, $awal, $akhir) {
    return $this->db->query(
        "SELECT *
        FROM ".$table."
        JOIN lelang ON ".$table.".id_lelang=lelang.id_lelang
        JOIN barang ON ".$table.".id_barang=barang.id_barang
        JOIN masyarakat ON ".$table.".id_user=masyarakat.id_user
        WHERE ".$table.".tgl
        BETWEEN '".$awal."'
        AND '".$akhir."'"
    )->getResult();
...
public function filter($table, $awal, $akhir)
	{
		if (!empty($awal) && !empty($akhir)) {
			...
			->where('tanggal_transaksi >=', $awal)
			->where('tanggal_transaksi <=', $akhir)
			->get()
			->getResult();
		} else {
			return [];
...
// 	}
	...
	public function edit_lelang($table1, $table2, $table3, $table4, $on, $on2, $on3, $where)
	{
		return $this->db->table($table1)->join($table2, $on)->join($table3, $on2)->join($table4, $on3)->getWhere($where)->getRow();
	}
	public function edit_history($table1, $table2, $table3, $table4, $on, $on2, $on3, $where)
	{
		return $this->db->table($table1)->join($table2, $on)->join($table3, $on2)->join($table4, $on3)->getWhere($where)->getRow();
	}
